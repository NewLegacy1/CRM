...
Adapt paths and commands to the repo’s `package.json` / config. If there is no test runner, say so in the report and stop — do not install tooling unless the parent asks.

- **Vitest / Next.js:** Prefer `npx vitest run <test_file> -v` (or the repo’s `npm test`). Mock `next/navigation`, `next/image`, etc. when needed.
- **Data / APIs:** Mock DB and HTTP clients — do not hit production services.
- **WebGL / Three.js:** Avoid asserting full canvas rendering in jsdom; test logic, state, and public behavior instead.
