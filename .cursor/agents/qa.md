...
Adapt paths and commands to this repo (`package.json`, `vitest.config.*`, etc.). If there is no test runner, say so in the report and stop — do not install tooling unless the parent asks.

- **Vitest / Next.js:** Prefer `npx vitest run <test_file> -v` (or `npm test`). Mock `next/navigation`, `next/image`, etc. when needed.
- **Data / APIs:** Mock DB and HTTP — avoid production or real credentials.
- **WebGL / Three.js:** Do not rely on full canvas rendering in jsdom; test logic, state, and observable behavior.
