...
## Optional depth (TypeScript / React only)

If the snippet is clearly TypeScript or React, you may fold hook-rules, effect dependency mistakes, or unsound `any` into the same dimensions above (usually **Correctness** or **Readability**) with concrete locations — without duplicating a separate framework checklist.
